...
fn matching_configured_version_produces_no_warning() {
    let engine = GuideEngine::load_directory(DATA_DIRECTORY, Some("1.0.3".to_string()))
...
fn version_info_present_in_every_answer() {
    ...
    assert_eq!(item.version.knowledge_version, "1.0.3");

    let pal = engine.lookup_pal("lamball");
    assert_eq!(pal.version.knowledge_version, "1.0.3");
...
fn configured_version_propagates_to_all_answer_types() {
    ...
    let pal = engine.lookup_pal("lamball");
    assert_eq!(
        pal.version.configured_game_version,
        Some("1.0.3".to_string())
    );
