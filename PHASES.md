...
Map knowledge enrichment: M0 and M1 are complete. The reviewed store now contains 2 logical maps, 123 localized regions, 160 map points (152 fast-travel and 8 boss towers), and 67,952 Pal habitat zones. Structured habitat references cover 282 of 299 canonical Pals after the spawner join was widened: all wild-spawner rows per placement name are now emitted (previously only one row per name), and unplaced spawners resolve coordinates through the boss-location table or biome-area estimation. The remaining 17 Pals are raid bosses, tower/legendary encounters, or breeding-limited variants with no wild field spawner. 71 unmatched placements and 678 unresolved raw Pal references remain explicit intake exclusions. Guide tools expose coordinate location, nearest reviewed anchors, and ranked Pal spawn zones. Region answers remain approximate until reviewed boundary geometry is available; routes are not yet implemented and material/resource locations remain out of scope until separately reviewed.
