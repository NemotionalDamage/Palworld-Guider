...
Phase G3 requires a sixth audit correction. The bounded Rust agent loop, typed registry, deterministic calculators, Tantivy retrieval, provider adapters, provenance, and offline tests are present, and the fifth-correction regressions pass. Remaining bypasses let a comma-separated numeric claim reuse a prior clause's entity evidence, and let explicit unknown breeding answers assert a permitted parent without a copula. Semantic vector search remains deferred.
...
- Expanded the final-answer grounding gate to parse complete scaled, fractional, scientific-notation, and question-locale Chinese quantity expressions, pair numeric values with their exact result entities, bind breeding conclusions or explicit unknowns to the requested parent pair and successful result, bind numeric claims to the entity named in their immediate claim context, and reject offspring assertions after explicit unknown breeding answers; a sixth audit found comma-separated claim leakage and copula-less offspring assertions.
- Kept exact and structured lookup ahead of lexical retrieval and deferred semantic vector search.

### Verification

`cargo fmt --all -- --check`, `cargo clippy --all-targets -- -D warnings`, `cargo test` with 106 tests, and `git diff --check` all passed during the sixth audit. Three additional temporary acceptance regressions reproduced grounding bypasses and were removed after their failure output was captured. Detailed evidence remains in `docs/phase-records/phase-g3.md`.

### Blockers

The numeric claim-context window is bounded by a connector-word list and punctuation checked only before scanned tokens, so a comma-separated clause (`You need 5 Wood, 5 Wool ...`) leaks the prior entity into the next claim's context and reuses its same-value evidence. The breeding unknown check rejects only copula-anchored offspring assertions; copula-less forms (`Unknown result: the offspring Lamball.`, `Unknown result: Lamball.`) still pass with a visible offspring guess. These fail the requirement that model-generated arithmetic and breeding guesses never become user-visible facts.

Next required action: make the numeric claim-to-entity binding structural (the claim must be adjacent to the entity it names, independent of a connector-word list), and reject any permitted entity in result-assertion position for explicit unknown breeding answers, adding permanent regressions for the three bypasses.
