...
Phase G3 is complete after a fifth audit correction. The bounded Rust agent loop, typed registry, deterministic calculators, Tantivy retrieval, provider adapters, provenance, and offline tests are present, and the fifth-correction regressions pass. Numeric claims bind to the entity named in their immediate claim context instead of any same-sentence entity, and explicit unknown breeding answers allow only uncertainty language, never offspring assertions. Semantic vector search remains deferred.
