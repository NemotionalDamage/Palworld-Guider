...
- deferred byproduct credits that never pre-deduct a recipe's own ingredients
- first-batch bootstrap seed accounting when a recipe byproduct is also its ingredient
...
- Shortage calculations consume stocked target and intermediate items, credit same-item and cross-item byproducts only after a craft completes, and then expand remaining missing quantities into raw-material shortages; sibling subtrees are processed producer-before-consumer so credits apply regardless of ingredient declaration order.
- Craftable counts describe additional items, consume intermediate inventory through the dependency tree, credit recipe byproducts with the same order-independent deferred sibling processing, report boundary-limiting materials, and reject representational overflow.
- A recipe byproduct that is also an ingredient of the same recipe requires an external seed for the first batch; later batches reuse the byproduct, so the ingredient demand scales as `b + (N-1) * max(0, b-c)` per batch quantities `b` and `c`.
...
- A self-deduction regression initially let a recipe's own byproduct cover its own ingredients, under-reporting a required seed material and over-reporting craftable counts; root and intermediate byproduct cases now require the first-batch seed and reuse byproducts from the second batch onward.

Final gates:

- `cargo fmt --all -- --check` passed.
- `cargo clippy --all-targets -- -D warnings` passed.
- `cargo test` passed with 40 tests.
